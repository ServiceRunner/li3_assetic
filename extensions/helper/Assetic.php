<?php
/**
 * li3_assetic: Assetic #li3 Plugin
 *
...
class Assetic extends \lithium\template\Helper
{
    ...
    public function _init()
    {
        ...
        if (!static::$config) {
            // Get initial config from bootstrap
            static::config(Libraries::get('li3_assetic'));
        }
    ...
    public function styles($options = array())
    {
        ...
        return $this->_context->helper('html')->style($options['target'] . ($options['version'] ? '?' . $options['version'] : ''), array('inline' => true));
